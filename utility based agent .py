...
memory = [[-1 for _ in range(GRID_SIZE)] for _ in range(GRID_SIZE)]  # clean = 0, dirty = 1 
...
pygame.display.set_caption("Utility-Based Vacuum Cleaner Agent")
...
while running:
        ...
        if event.type == pygame.QUIT:
            ...
            if event.key == pygame.K_RETURN and not all_clean():
                ...
                if best_cell:
                    vx, vy = best_cell
                    move_count += 1
                    log.append(f"Agent moved to ({vx}, {vy}) with utility {round(best_score, 2)}")
    ...
    if all_clean() and not room_cleaned_popup_shown:
        ...
        popup_rect = pygame.Rect(WINDOW_WIDTH // 4, WINDOW_HEIGHT // 3, WINDOW_WIDTH // 2, WINDOW_HEIGHT // 6)
