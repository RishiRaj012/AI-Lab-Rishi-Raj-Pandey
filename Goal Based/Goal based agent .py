...
pygame.display.set_caption("Goal-Based Vacuum Cleaner Agent")
...
def find_nearest_dirty(start_x, start_y):
        ...
        if event.type == pygame.QUIT:
            ...
            if event.key == pygame.K_RETURN and not all_clean():
                if room[vy][vx] == 1:
                    room[vy][vx] = 0
                    memory[vy][vx] = 1
                    log.append(f"Cleaned cell ({vx}, {vy})")
                else:
                    if memory[vy][vx] == -1:
                        memory[vy][vx] = 0
                    log.append(f"At clean cell ({vx}, {vy})")

                # find new goal if no any path is found
                if not path_to_goal:
                    path_to_goal = find_nearest_dirty(vx, vy)

                # another step
                if path_to_goal:
                    next_x, next_y = path_to_goal.pop(0)
                    vx, vy = next_x, next_y
                    move_count += 1
                    log.append(f"Moved to ({vx}, {vy})")
